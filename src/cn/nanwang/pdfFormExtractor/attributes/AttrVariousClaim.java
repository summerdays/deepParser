...
public class AttrVariousClaim extends PDFAttribute {
	...
	public AttrVariousClaim() {
		...
		name = "Credit Card Claim \t Medical Claim \t Insurance Claim \t Charge CLaim \t Student Loan \t Other Loan \t Account Claim";
		page = 7;
		isDebug = false;
	}
	...
	public void computeValue(String src){
		...
		for(int ipart = 0; ipart < parts.length; ipart++){
			...
			if( parts[ipart].indexOf("credit card") >=0 
					|| parts[ipart].indexOf("creditcard") >=0 
					|| parts[ipart].indexOf("credit account") >=0 
					|| parts[ipart].indexOf("line of credit") >=0 
					|| parts[ipart].indexOf("visa") >=0 
					|| parts[ipart].indexOf("master") >= 0
					|| parts[ipart].indexOf("american express") >= 0
					|| parts[ipart].indexOf("general merchandise") >= 0
					|| parts[ipart].indexOf("charge card") >= 0
					|| parts[ipart].indexOf("charge account") >= 0
					){
					this.creditClaim = accumValue(this.creditClaim, words[words.length-1].replaceAll("[$,]", ""));					
					if(isDebug)
						System.out.printf("%s\t%s\n",words[words.length-1].replaceAll("\\D", ""), this.creditClaim);
					continue;
			}
			
			if( parts[ipart].indexOf("medical") >=0 ||  parts[ipart].indexOf("health") >= 0 				
					 || parts[ipart].indexOf("hospital") >=0){
				this.medicalClaim = accumValue(this.medicalClaim, words[words.length-1].replaceAll("[$,]", ""));
				continue;
			}
			
			if(parts[ipart].indexOf("insurance") >= 0){
				this.insuranceClaim = accumValue(this.insuranceClaim, words[words.length-1].replaceAll("[$,]", ""));
				continue;
			}
			...
			if(parts[ipart].indexOf("chargeprivilege") >= 0 
				...
				this.chargeClaim = accumValue(this.chargeClaim, words[words.length-1].replaceAll("[$,]", ""));
				continue;
			}
			
			if(parts[ipart].indexOf("student loan") >= 0  ){
				this.studentloan = accumValue(this.studentloan, words[words.length-1].replaceAll("[$,]", ""));
				continue;
			}
			...
			if(parts[ipart].indexOf("account") >= 0){	
				this.accountClaim = accumValue(this.accountClaim, words[words.length-1].replaceAll("[$,]", ""));
				continue;
			}
		}
