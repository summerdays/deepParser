...
public class AttrBusinessRealProperty extends PDFAttribute {
  ...
  static Pattern twoMoneyRegex =
      Pattern
          .compile("(?:^|[\\D&&[^\\.]]+)([0-9]{1,3}(?:,[0-9]{3}){1,4}(?:\\.[0-9]{2})?)[\\s\\$\\D]+([0-9]{1,3}(?:,[0-9]{3}){1,4}(?:\\.[0-9]{2})?).*");
    ...
    if (src.contains("usiness")) {
      ...
      if (m.find()) {
        if (businessRealPropertyValue == null) {
          ...
          double v1 = Double.valueOf(businessRealPropertyValue.replaceAll(",", ""));
          double s1 = Double.valueOf(businessRealPropertySecured.replaceAll(",", ""));
          double v2 = Double.valueOf(m.group(1).replaceAll(",", ""));
          double s2 = Double.valueOf(m.group(2).replaceAll(",", ""));
          v1 = v1 + v2;
          s1 = s1 + s2;
